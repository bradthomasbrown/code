...
Deno.test('compile ERC20 exclude PUSH0', async () => {
    const results = await compile({ ...params['ERC20'], excludeOpcodes: ['PUSH0'] })
...
Deno.test('compile Resolver exclude PUSH0', async () => {
    const results = await compile({ ...params['Resolver'], excludeOpcodes: ['PUSH0'] })
...
Deno.test('compile UniversalRouter exclude PUSH0', async () => {
    const results = await compile({ ...params['UniversalRouter'], excludeOpcodes: ['PUSH0'] })
