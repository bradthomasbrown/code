...
const cache = new Cache(
  fromFileUrl(import.meta.resolve(`../../../solc/compile/fixtures`)),
);
...
Deno.test("generate and validate", async (t) => {
  ...
  await t.step("validate", async (t) => {
    ...
    for (const [sourceName, source] of sources) {
      await sourceCreated(
        t,
        context.set("source", `${path}/${sourceName}`),
        source,
      );
    }
  });
...
async function sourceCreated(
  ...
  for (const [contractName, contract] of contracts) {
    await contractCreated(
      t,
      context.set("contract", `${path}/${contractName}`),
      contract,
    );
  }
}
...
async function contractCreated(
  ...
  await t.step(
    "contract dir exists",
    async () => await contractDirExists(context),
  );
  ...
  if (bytecode) {
    ...
    if (linkReferences) {
      ...
      await t.step(
        "bytecode function file exists",
        async () => await bytecodeFunctionFile(context),
      );
    } else {
      await t.step(
        "bytecode string file exists",
        async () => await bytecodeStringFile(context),
      );
    }
  }
}
...
async function linkFile(context: Context): Promise<void> {
  ...
  const { link } = <typeof import("../templates/_link.ts")> await import(
    `${path}/_link.ts`
  );
...
async function refsFile(context: Context): Promise<void> {
  ...
  const { refs } = <typeof import("../templates/_refs.ts")> await import(
    `${path}/_refs.ts`
  );
...
async function bytecodeStringFile(context: Context): Promise<void> {
  ...
  assert(path, "contract path not set");
  assert(
    await exists(`${path}/_bytecodeString.ts`),
    "bytecode string file not found",
  );
  const { bytecode } =
    <typeof import("../templates/_bytecodeString.ts")> await import(
...
async function bytecodeFunctionFile(context: Context): Promise<void> {
  ...
  assert(path, "contract path not set");
  assert(
    await exists(`${path}/bytecode.ts`),
    "bytecode function file not found",
  );
  const { bytecode } = <typeof import("../templates/bytecode.ts")> await import(
    `${path}/bytecode.ts`
  );
