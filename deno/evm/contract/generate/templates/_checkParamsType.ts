...
export function checkParamsType(params: ByIndex | [ByName], inputs: Input[]): ParamsKind
{
    if (inputs.length == 1 && inputs[0].type == 'tuple')
        throw new Error('currently unhandled ambiguity: first and only input is a struct')
    
    const [param] = params
    if (inputs.length == 1 && param instanceof Object && !(param instanceof Array))
