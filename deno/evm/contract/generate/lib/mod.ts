...
async function createSource(sourceKey: Deno.KvKey): Promise<void> {
  const { value } = await kv.get<{ dir: string; contracts: Contracts }>(
    sourceKey,
  );
...
async function createContract(contractKey: Deno.KvKey): Promise<void> {
  const { value } = await kv.get<{ dir: string; contract: Contract }>(
    contractKey,
  );
  ...
  if (linkReferences) {
    ...
    await templar.generate(
      ...
      [{
        type: "expression",
        id: "bytecode",
        content: JSON.stringify(bytecode),
      }],
      ...
      [
        {
          type: "expression",
          id: "link",
          content: `await import('../_link.ts')`,
        },
        {
          type: "expression",
          id: "bytecodeString",
          content: `await import('./_bytecodeString.ts')`,
        },
        {
          type: "expression",
          id: "refs",
          content: `await import('./_refs.ts')`,
        },
...
function linkReferencesToRefs(
  linkReferences: ExtendedMap<string, ExtendedMap<string, Offsets>>,
): Refs {
  return linkReferences
    .foldlWithKey((sourceName, libraries, refs) =>
      libraries
        .foldlWithKey((libraryName, _offsets, refs) => {
          const library = libraryName;
          const placeholder = `__$${
            keccak(`${sourceName}:${libraryName}`).slice(0, 34)
          }$__`;
...
async function handleDescriptorSet(
  descriptorSetKey: Deno.KvKey,
): Promise<void> {
  ...
  assert(
    descriptorSet.every(({ type }) => (type ?? "function") == setType),
    "descriptor set contains mixed types",
  );
