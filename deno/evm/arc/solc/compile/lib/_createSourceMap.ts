import { walk, WalkOptions } from "https://deno.land/std@0.224.0/fs/walk.ts";
...
export async function createSourceMap({
  ...
  const normalizedRemappings =
    remappings?.map((remapping) => remapping.split("=")) ?? [];
  ...
  const applyRemappings = (source: string) =>
    normalizedRemappings.reduce(
      (source, [from, to]) => source.replace(from, to),
      source,
    );
  const pathToSource = (path: string, root: string) =>
    path.replace(`${root == basePath ? root : dirname(root)}/`, "");
  const codeToImportLines = (code: string): string[] =>
    code.match(/^import\b.+?(['"])(.+)\1/mg) ?? [];
  const importLineToImportPath = (importLine: string) =>
    applyRemappings(importLine.match(/^import\b.+?(['"])(.+)\1/)![2]);
  const codeToImportPaths = (code: string) =>
    codeToImportLines(code).map(importLineToImportPath);
  const sourceResolve = (rootSource: string, pathSegment: string) =>
    resolve(dirname(`/${rootSource}`), pathSegment).replace(/^\//, "");
  const isAbsoluteSource = (source: string) => !source.match(/^\./);
  const sourceResolver = (rootSource: string) => (pathSegment: string) =>
    isAbsoluteSource(pathSegment)
      ? pathSegment
      : sourceResolve(rootSource, pathSegment);
  const getImportSources = (code: string, rootSource: string) =>
    codeToImportPaths(code).map(sourceResolver(rootSource));
  ...
  const fulfillSourceRequirement = async (source: string) => {
    ...
    for (const importSource of getImportSources(code, source)) {
      if (!sourceToPathMap.has(importSource)) {
        requiredSourcesSet.add(importSource);
      } else if (!sourceToCodeMap.has(importSource)) {
        await fulfillSourceRequirement(importSource);
      }
    }
  ...
  const walkRoot = async (root: string) => {
    ...
    for await (const { path } of walk(root, walkOptions)) {
      if (!await handlePath(path, root)) return true;
    }
  };
