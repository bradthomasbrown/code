...
declare const __nat: unique symbol;
declare const s: unique symbol;

interface NatBase {
    readonly __nat: typeof __nat;
}

interface NatZero extends NatBase {}

interface NatNonzero extends NatZero {
    readonly value: [typeof s, ...typeof s[]];
}

type Nat = NatZero | NatNonzero;

type S<N extends Nat> =
    NatBase & (N extends NatNonzero
        ? { value: [...N["value"], typeof s] }
        : { value: [typeof s] })

// opposite of successor, returns never for zero or N - 1 for nonzero
type R<N extends Nat> =
    N extends NatNonzero
        ? N["value"] extends [typeof s]
            ? NatZero
            : N["value"] extends [typeof s, ...infer Rest extends typeof s[]]
                ? NatBase & { value: Rest }
                : never
        : never
...
// type OneA = R<Two> // value: [typeof s];
// type ZeroA = R<OneA> // type ZeroA = NatZero
...
// type Zero = MakeNat<[]>; // type Zero = NatZero
// type One = S<Zero>; // type One = NatBase & { value: [typeof s]; }
// type Two = S<One>; // type Two = NatBase & { value: [typeof s, typeof s]; }


type Add<A extends Nat, B extends Nat, Acc extends typeof s[] = []> =
    B extends NatNonzero
        ? Add<A, NatZero, [...B["value"]]> // Move B's value to Acc
        : A extends NatNonzero
            ? MakeNat<[...Acc, ...A["value"]]> // Combine Acc and A's value
            : MakeNat<Acc>; // Both A and B are zero, return Acc

// type Zero = MakeNat<[]> // type Zero = NatZero
// type One = S<Zero> // type One = NatBase & { value: [typeof s]; }
// type ZeroA = Add<Zero, Zero> // type ZeroA = NatZero
// type OneA = Add<Zero, One> // type OneA = NatBase & { value: [typeof s]; }
// type OneB = Add<One, Zero> // type OneB = NatBase & { value: [typeof s]; }
// type Two = Add<One, One> // type Two = NatBase & { value: [typeof s, typeof s]; }

// multiplies, can be optimized by ensuring B is smallest, also other weird optimizations exist
type Mul<A extends Nat, B extends Nat, Acc extends Nat = NatZero> =
    A extends NatNonzero
        ? B extends NatNonzero
