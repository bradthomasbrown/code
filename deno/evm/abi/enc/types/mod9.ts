...
class _String implements Type {
    ...
    private _enc: undefined | string
    get enc(): string {
        return this._enc ??= new _Bytes(undefined, new TextEncoder().encode(this.value)).enc
    }
...
class _Address implements Type {
    ...
    constructor(address?: string) {
        this.value = address ??= "0x0000000000000000000000000000000000000000"
...
class _Bytes extends Uint8Array implements Type {
    ...
    constructor()
    constructor(byteCount: undefined | number)
    constructor(byteCount: undefined | number, string: string)
    constructor(byteCount: undefined | number, iterable: Iterable<number>)
    constructor(byteCount: undefined | number, array: ArrayLike<number>)
    constructor(byteCount: undefined | number, buffer: ArrayBufferLike, byteOffset?: number, length?: number)

    constructor(
        public readonly byteCount?: number,
        ...args: [] | [string] | [Iterable<number>] | [ArrayLike<number>] | [ArrayBufferLike, number?, number?]
    ) {
        let arr0: undefined | Uint8Array
        if (byteCount) {
            assert(Number.isInteger(byteCount), `%m not integer, ${byteCount}`)
            assert(0 < byteCount && byteCount <= 32, `%m not in range (0 < m <= 32), ${byteCount}`)
            arr0 = new Uint8Array(byteCount)
        }

        const [arg0, byteOffset, length] = args
        let arr1: Uint8Array

        if (isUndefined(arg0)) arr1 = new Uint8Array()
        else if (isString(arg0)) arr1 = decodeHex(arg0.replace(/^0x/, ''))
        else if (isIterable(arg0)) arr1 = new Uint8Array(arg0)
        else if (isArrayLike(arg0)) arr1 = new Uint8Array(arg0)
        else if (isArrayBufferLike(arg0)) arr1 = new Uint8Array(arg0, byteOffset, length)
        else assertNever(arg0, 'exhaustive check')

        if (arr0) arr0.set(arr1)
        else arr0 = arr1
        super(arr0)
        
        this.sym = typeIdMap.blaze(["_Bytes", byteCount])
    }
    ...
    private _isDynamic: undefined | boolean
    get isDynamic(): boolean {
        return this._isDynamic ??= this.byteCount === undefined
    }
