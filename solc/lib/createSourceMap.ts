...
export async function createSourceMap({
    ...
    requiredSources: string|[string, ...string[]]
    basePath?: string
    includePaths?: string|string[]
    excludePaths?: string[]
    remappings?: string|string[]|string[][]
}) {

    const normalizedRequiredSources = typeof requiredSources == 'string'
        ? requiredSources.split(',')
        : requiredSources
    const normalizedIncludePaths = typeof includePaths == 'string'
        ? includePaths.split(',')
        : includePaths
        ?? []
    const normalizedRemappings = typeof remappings == 'string'
        ? remappings.split(',').map(remapping => remapping.split('='))
        : typeof remappings?.[0] == 'string' ? (remappings as string[]).map(remapping => remapping.split('='))
        : (remappings as string[][])
        ?? []
    const roots = [basePath ?? Deno.cwd(), ...(normalizedIncludePaths ?? [])]
    const requiredSourcesSet = new Set<string>([...normalizedRequiredSources])
